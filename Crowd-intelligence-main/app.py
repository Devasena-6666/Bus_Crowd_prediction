...
genai.configure(api_key="AIzaSyC_kiMjhs3ct1d3_0sN5pCRVts8HDz_7gA")
...
# Live passenger counting state (in-memory, process-level).
passenger_count = 0
is_counting = False
counter_thread = None
...
def _crowd_status_from_count(count):
    if count <= 15:
        return "Low"
    if count <= 30:
        return "Medium"
...
def _count_worker():
    """Background worker that updates the shared passenger count."""
    global passenger_count, is_counting
...
def start_count():
    return jsonify({"status": "ready", "message": "Send frames to /update_occupancy."})

@app.route("/stop_count", methods=["POST"])
def stop_count():
    return jsonify({"status": "stopped", "message": "Counting stopped."})

@app.route("/get_count", methods=["GET"])
def get_count():
    bus_no = request.args.get("bus_no", "unknown")
    if bus_no in bus_crowd_data:
        data = bus_crowd_data[bus_no]
        return jsonify({"count": data["passenger_count"], "status": data["status"]})
    return jsonify({"count": 0, "status": "Unknown"})
...
yolo_model = YOLO("yolov8n.pt")  # correct model name — auto-downloads if not present
...
def update_occupancy():
    try:
        ...
        passenger_count = len(boxes) if boxes is not None else 0
        ...
        return jsonify({"error": str(e), "passenger_count": 0, "status": "Error"}), 500
